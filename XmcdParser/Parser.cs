    ...
    public class Parser
    {
        ...
        public Parser()
        {
            ...
            Add("DISCID=(.+)", (disk, collection) =>
            {
                ...
                disk.FreeDBDiskIds.AddRange(strings.Select(x => x.Trim()));
            });
            ...
            {
                disk.Tracks = new List<string>(collection.Count);
                foreach (Match match in collection)
                {
                    int pos = 0; // supports line wraps and null values
                    if (int.TryParse(match.Groups[1].Value, out pos))
                    {
                        while (pos >= disk.Tracks.Count)
                        {
                            disk.Tracks.Add(null);
                        }
                        disk.Tracks[pos] += match.Groups[2].Value.TrimEnd();
                    }
                }
            });
